...
public class testScraper extends Application {
    private static final Map<String, Integer> numberChaps = new HashMap<>();
...
//        messages.forEach(System.out::println);
    ...
    private void doGetAllTocs(IntegerProperty property, String link) {
        TocProcessor processor = new NovelTocProcessor();
        processor.process(link);

        property.set(property.get() + 1);
//            System.out.println(property.getAll());
    }

    private void doGetAllChapters(String s) {
        try {
            ChapterSearchEntry entry = new ChapterSearchEntry(new CreationBuilder("moin").build(), Source.create(s, SourceType.START));
    ...
    private void browseGuiLess(List<String> chapters) {
        for (String chapter : chapters) {
            browse(chapter);
        }
    }

    private void browse(String chapter) {
//        new FirefoxDriver();
    }
...
//        System.out.println("New Link: " + detective.tryNextLink(s));
    ...
    private void tryIt(IntegerProperty integer, String s) throws URISyntaxException, IOException {
        Source source = Source.create(s, SourceType.START);
