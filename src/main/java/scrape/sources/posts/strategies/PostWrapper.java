...
import scrape.sources.novel.toc.strategies.intface.DocumentElement;
import scrape.sources.posts.strategies.intface.Filter;

/**
 *
 */
public enum PostWrapper implements DocumentElement, Filter {
    ...
    PostWrapper(String s) {
        selector = s;
    }

    public static PostWrapper tryAll(Document document) {
        Element child = null;
        PostWrapper result = null;
        for (PostWrapper postsWrapper : PostWrapper.values()) {
