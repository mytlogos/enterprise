...
public class XmlTocReader extends XmlAbstractToc implements TocReader {
    ...
    public TableOfContent read(String path) {
        try {
            ...
            return new TableOfContent("N/A", path);
        }
    }

    private TableOfContent getRoot(Document document, String path) {
        Element root = document.getRootElement();
        Element titleElement = root.getChild(titleTag);
        String title = titleElement.getTextNormalize();

        TableOfContent creationRoot = new TableOfContent(title, path);
    ...
    private <E> E handleAttr(Element section, String attr, FunctionEx<Attribute, E> function) {
