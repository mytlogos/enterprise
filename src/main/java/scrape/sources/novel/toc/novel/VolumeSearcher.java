...
import scrape.sources.novel.toc.Path;
import scrape.sources.novel.toc.PathFinder;
...
public class VolumeSearcher extends Searcher {
    private final String[] volumeTerms = new String[]{"volume", "arc", "book"/*, "vol."*/};
    ...
    private List<TableOfContent> getVolumeTocs() {
    ...
    private List<TableOfContent> getVolumeTocs(VolumeManager container, Element firstCommonParent) {
        container.resolve(container.getCommonPath());

        Map<String, List<Path>> resolvedMappedPaths = container.getResolvedMappedPaths();

        List<TableOfContent> tocs = new ArrayList<>();
    ...
    private TableOfContent getVolumeToc(Element firstCommonParent, Map<String, List<Path>> resolvedMappedPaths, String key) {
    ...
    private TableOfContent buildToc(SortedMap<Element, SortedMap<Element, List<Element>>> map) {
